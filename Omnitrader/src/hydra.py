...
class Hydra:
    ...
    def __init__(self, config_path: str = None, *, striker: float = None, foundation: float = None, moat: float = None, config: dict = None, circuit_breaker: object = None):
        ...
        """
        # Initialize circuit breaker early to handle both code paths
        if circuit_breaker is not None:
            self.circuit_breaker = circuit_breaker
        elif config is not None:
            # Load path: no fresh circuit breaker, use passed-in or create minimal
            self.circuit_breaker = circuit_breaker
        else:
        ...
        if config is not None:
            ...
            # Initialize circuit breaker for load() path too
            ...
            # Circuit breaker initialization (only when creating fresh instance)
    ...
    def get_balance(self, pool: str) -> float:
        ...
        """
        pool = pool.lower()
        valid_pools = ("moat", "striker", "foundation", "dao_treasury", "foundation_research", "foundation_legal")
    ...
    def update_balance(self, pool: str, amount: float) -> float:
        ...
        try:
            ...
            # Safety floor: prevent pools from going negative
            if record.balance < 0:
                logger.warning("Pool '%s' floored at $0 (was %.2f)", pool, record.balance)
                record.balance = 0.0
    ...
    def check_circuit_breaker(self, telegram_alert_fn=None) -> dict:
        ...
        """
        if self.circuit_breaker is None:
            return {"breaker_triggered": False, "message": "Circuit breaker not initialized"}
    ...
    def can_trade(self, striker_capital: float = None) -> bool:
        ...
        if cb_status.get("breaker_triggered"):
            threshold_pct = (self.circuit_breaker.threshold * 100) if self.circuit_breaker else 0
