...
async def get_status(
    ...
    try:
        ...
        cb_status = {}
        try:
            cb_status = hydra.get_circuit_breaker_status()
        except Exception as e:
            logger.warning("Failed to get circuit breaker status: %s", e)
            cb_status = {"breaker_triggered": False, "message": "Circuit breaker not initialized"}
        ...
        return StatusResponse(
            ...
            breaker_tripped=cb_status.get("breaker_triggered", False),
        )
...
# ── Draft Bounty Report Endpoints ────────────────────────────────

@router.post("/sleuth/generate-draft")
async def generate_draft_report(
    req: SubmitAlertRequest,
    key: str = Depends(api_key_auth),
) -> Dict:
    """Generate a draft bounty report from an on-chain alert.

    Creates a PDF report, records it as DRAFT, and links to the alert.

    Args:
        req: Alert ID and target.
    """
    try:
        from ..sleuth.bounty_reporter import BountyReporter
        from ..utils.db import OnChainAlert, get_session
        ...
        session = get_session()
        try:
            alert = session.query(OnChainAlert).filter_by(alert_id=req.alert_id).first()
            ...
            if not result:
                raise HTTPException(
                    status_code=500,
                    detail="Failed to generate draft report",
                )
        ...
        logger.error("Generate draft error: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/sleuth/draft-reports")
async def list_draft_reports(
    key: str = Depends(api_key_auth),
    limit: int = Query(50, ge=1, le=1000),
) -> Dict:
    """List all draft bounty reports.
    ...
    try:
        ...
        reporter = BountyReporter.load()
        drafts = reporter.get_draft_reports()[:limit]
        ...
        logger.error("List drafts error: %s", e)
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/sleuth/draft-reports/{report_id}/approve")
async def approve_draft_report(
    report_id: int,
    key: str = Depends(api_key_auth),
) -> Dict:
    """Approve a draft bounty report.
    ...
    try:
        ...
        logger.error("Approve report error: %s", e)
        raise HTTPException(status_code=500, detail=str(e))
...
@router.post("/dao/propose")
async def dao_propose(
    key: str = Depends(api_key_auth),
    title: str = Body(..., description="Proposal title"),
    description: str = Body(..., description="Proposal description"),
    proposal_type: str = Body("fund_allocation", regex="^(fund_allocation|target_selection|code_upgrade)$"),
    metadata: Dict = Body(default_factory=dict),
) -> Dict:
    """Propose a new DAO proposal.
    ...
    """
    try:
        from ..dao.dao_contracts import DAOContractInterface

        dao = DAOContractInterface()
        result = await dao.create_proposal(
            title=title,
            description=description,
            proposal_type=proposal_type,
            proposer="omnitrader-api",
            metadata=metadata,
        )
        return {
        ...
        logger.error("DAO propose error: %s", e)
        raise HTTPException(status_code=500, detail=str(e))
