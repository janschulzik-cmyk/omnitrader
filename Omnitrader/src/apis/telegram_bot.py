...
class TelegramBot:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.commands = {
            ...
            "review_reports": "Review draft bounty reports",
            "approve_report": "Approve a draft report (use /approve_report <id>)",
...
"""
    ...
    async def cmd_review_reports(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> None:
        """Handle /review_reports command."""
        if not self._check_admin(update):
            return
        ...
        if not drafts:
            self._send_message(
                update.effective_chat.id,
                "📋 No draft bounty reports found.",
            )
            return

        lines = ["*Draft Bounty Reports*"]
        for d in drafts:
            lines.append(
                f"  ID: {d['id']} | Target: {d.get('submitted_to', '?')} "
                f"| Event: {d.get('event_type', '?')}"
            )
        lines.append("")
        lines.append("Use /approve_report <id> to approve a report.")

        self._send_message(update.effective_chat.id, "\n".join(lines))

    async def cmd_approve_report(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> None:
        """Handle /approve_report <id> command."""
        if not self._check_admin(update):
            return

        # Get report ID from command args
        if not context.args:
            self._send_message(
                update.effective_chat.id,
                "⚠️ Usage: /approve_report <id>\n\n"
                "Run /review_reports to list draft reports.",
            )
            return
        ...
        try:
            ...
            self._send_message(
                update.effective_chat.id,
                f"⚠️ Invalid report ID: {context.args[0]}. Use a number.",
            )
            return
        ...
        if result:
            ...
            self._send_message(
                update.effective_chat.id,
                f"⚠️ Could not approve report {report_id}. Not found or error.",
            )
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            self.app.add_handler(CommandHandler("review_reports", self.cmd_review_reports))
            self.app.add_handler(CommandHandler("approve_report", self.cmd_approve_report))
